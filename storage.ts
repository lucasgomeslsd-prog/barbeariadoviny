...
export async function syncWithServerDatabase(): Promise<void> {
  try {
    ...
    if (db.config) {
      localStorage.setItem(CONFIG_KEY, JSON.stringify(db.config));
    }
...
export function getStoredConfig(): BarbershopConfig {
  try {
    ...
    if (!raw) {
      localStorage.setItem(CONFIG_KEY, JSON.stringify(DEFAULT_BARBERSHOP_CONFIG));
      return DEFAULT_BARBERSHOP_CONFIG;
    }
    const parsed = JSON.parse(raw);
...
export function saveStoredConfig(config: BarbershopConfig): void {
  try {
    ...
    saveConfigToFirestore(config).catch(() => {});
